 ...
 * @file cnab_tvh.c
 *
 * @brief Uses TV-H^{-1} inpainting with a linearly stabilized CNAB time stepping scheme to inpaint an image
 *
...
float* cnab_tvh(const float* image, const float* mask);
...
float* cnab_tvh(const float* image, const float* mask) {
    int i, j, c;
    float stall, change = 1.0;
    float *u, *uold, *v, *w, *absg, *f, *f1, *ft, *g, *tmpcpy;
    ...
    ft      = (float*) xmalloc(sizeof(float)*width*height*3);
    ...
    for(c = 0; c < 3; c++) {
        ...
        for(i = 0; i < height; i++) {
            for(j = 0; j < width; j++) {
                ...
                ft[chan+i*width+j] = (dxx(uold+chan, i, j)*(w[chan+i*width+j]*w[chan+i*width+j]+ DPAD*DPAD) 
...
if(it > 0) {
#pragma omp parallel for \
	shared(uold,v,w,absg,f,f1,mask,image,l0,dt) \
	private(c,i,j,chan)     
    for(c = 0; c < 3; c++) {
        chan = c*width*height;
        for(i = 0; i < height; i++) {
            for(j = 0; j < width; j++) {
                f[chan+i*width+j] = -dxx(ft+chan, i, j) - dyy(ft+chan, i, j)
                ...
                g[chan+i*width+j] = -p1*(dxxxx(uold+chan, i, j) + dyyyy(uold+chan, i, j) + 2*dxxdyy(uold+chan, i, j)) 
                -p0*l0*uold[chan+i*width+j];
    ...
    for(c = 0; c < 3; c++) {
        ...
        for(i = 0; i < height; i++) {
            for(j = 0; j < width; j++) {
                ...
                in[c][i*width+j] = uold[chan+i*width+j] + dt*f[chan+i*width+j];
            }
        }
