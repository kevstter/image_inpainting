 ...
 * @file sbdf2_tvh.c
 *
 * @brief Uses TV-H^{-1} inpainting with a linearly stabilized SBDF2 time stepping scheme to inpaint an image
 *
...
float* sbdf2_tvh(const float* image, const float* mask);
...
#pragma omp parallel for \
    shared(u,u2,v,w,absg) \
    private(c,i,j,chan)
    for(c = 0; c < 3; c++) {
        chan = c*width*height;
        for(i = 0; i < height; i++) {
            for(j = 0; j < width; j++) {
                u2[chan+i*width+j] = (dxx(u+chan, i, j)*(w[chan+i*width+j]*w[chan+i*width+j]+ DPAD*DPAD) 
...
#pragma omp parallel for \
    ...
    for(c = 0; c < 3; c++) {
        ...
        for(i = 0; i < height; i++) {
            for(j = 0; j < width; j++) {
                f[chan+i*width+j] = -dxx(u+chan, i, j) - dyy(u+chan, i, j)
                + p1*(dxxxx(u1+chan, i, j) + dyyyy(u1+chan, i, j) + 2*dxxdyy(u1+chan, i, j))
...
#pragma omp parallel for \
    ...
    for(c = 0; c < 3; c++) {
        ...
        for(i = 0; i < height; i++) {
            for(j = 0; j < width; j++) {
                f1[chan+i*width+j] = -dxx(u2+chan, i, j) - dyy(u2+chan, i, j)
                + p1*(dxxxx(u+chan, i, j) + dyyyy(u+chan, i, j) + 2*dxxdyy(u+chan, i, j))
                + p0*l0*u[chan+i*width+j]
