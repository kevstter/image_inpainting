...
inline float dxxxx(const float *x, int i, int j) {
    return x[In(i,j+2)] - 4*x[In(i,j+1)] + 6*x[In(i,j)] -4*x[In(i,j-1)] + x[In(i,j-2)];
}
inline float dyyyy(const float *x, int i, int j) {
    return x[In(i+2,j)] - 4*x[In(i+1,j)] + 6*x[In(i,j)] -4*x[In(i-1,j)] + x[In(i-2,j)];
}
inline float dxxdyy(const float *x, int i, int j) {
    return x[In(i+1,j+1)] + x[In(i+1,j-1)] + 4*x[In(i,j)] + x[In(i-1,j+1)] + x[In(i-1,j-1)] - 2*(x[In(i+1,j)] + x[In(i-1,j)] + x[In(i,j+1)] + x[In(i,j-1)]);
}
