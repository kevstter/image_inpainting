 ...
 * @file sbdf1_tv.c
 *
 * @brief Uses TV inpainting with a linearly stabilized semi-implicit Euler time stepping scheme to inpaint an image
 *
...
float* sbdf1_tv(const float* image, const float* mask);
...
#pragma omp parallel for \
    ...
    for(c = 0; c < 3; c++) {
        ...
        for(i = 0; i < height; i++) {
            for(j = 0; j < width; j++) {
                // in[c][i*width+j] = (1 + p0*dt*l0)*u[chan+i*width+j] 
                // + dt*(dxx((u+chan), i, j)*(w[chan+i*width+j]*w[chan+i*width+j]+ DPAD*DPAD) 
                ...
                // - (p1*dt)*(dxx((u+chan), i, j) + dyy((u+chan), i, j))
                // + dt*l0*mask[chan+i*width+j]*(image[chan+i*width+j] - u[chan+i*width+j]);
                in[c][i*width+j] = u[chan+i*width+j] 
                + dt*((dxx((u+chan), i, j)*(w[chan+i*width+j]*w[chan+i*width+j]+ DPAD*DPAD) 
                + dyy((u+chan), i, j)*(v[chan+i*width+j]*v[chan+i*width+j] + DPAD*DPAD) 
                - 2*v[chan+i*width+j]*w[chan+i*width+j]*cdxy((u+chan), i, j)) / (absg[chan+i*width+j]*absg[chan+i*width+j]*absg[chan+i*width+j]) 
